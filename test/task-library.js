...
contract('TaskLibrary', function (accounts) {
  ...
  describe('when updating existing tasks', () => {
    ...
    it('should not interfere in "accepted", "eth" or "tokens" props', async function () {
      await colony.makeTask('TASK A', 'INTERESTING TASK SUMMARY');
      const prevAcceptedValue = await eternalStorage.getBooleanValue.call(sha3('task_accepted', 0));
      const prevEthBalance = await eternalStorage.getUIntValue.call(sha3('task_eth', 0));
      const prevTokensBalance = await eternalStorage.getUIntValue.call(sha3('task_tokensWei', 0));
      await colony.updateTaskTitle(0, 'TASK B');
      const name = await eternalStorage.getStringValue.call(sha3('task_name', 0));
