...
import { MINING_CYCLE_DURATION, DEFAULT_STAKE, REWARD, UINT256_MAX, MIN_STAKE, WAD, DISPUTE_DEFENCE_WINDOW } from "../../helpers/constants";
...
contract("Reputation mining - root hash submissions", (accounts) => {
  ...
  describe("when eliminating submissions", () => {
    ...
    it("should allow submitted hashes with multiple backers to go through multiple responses to a challenge", async () => {
      ...
      await forwardTime(MINING_CYCLE_DURATION / 6 + DISPUTE_DEFENCE_WINDOW + 1, this);
  ...
  describe("when rewarding and punishing good and bad submissions", () => {
    ...
    it("should reward all stakers if they submitted the agreed new hash", async () => {
      ...
      await forwardTime(MINING_CYCLE_DURATION / 2 + DISPUTE_DEFENCE_WINDOW, this);
      await forwardTime(MINING_CYCLE_DURATION / 2 + DISPUTE_DEFENCE_WINDOW + 1, this);
    ...
    it("should be able to complete a cycle and claim rewards even if CLNY has been locked", async () => {
      ...
      await metaColony.moveFundsBetweenPots(1, UINT256_MAX, 1, UINT256_MAX, UINT256_MAX, 1, 0, amount, clnyToken.address);
