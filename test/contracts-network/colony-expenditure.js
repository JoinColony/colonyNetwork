...
contract("Colony Expenditure", accounts => {
  ...
  describe("when updating expenditures", () => {
    ...
    it("should allow arbitration users to set the claimDelay", async () => {
      await checkErrorRevert(colony.setExpenditureClaimDelay(1, 0, expenditureId, SLOT0, SECONDS_PER_DAY, { from: ADMIN }), "ds-auth-unauthorized");
  ...
  describe("when claiming expenditures", () => {
    ...
    it.only("should allow anyone to claim on behalf of the recipient, with network fee deducted", async () => {
      const relayHub = await RelayHub.new();
      console.log("enablegsn");
      console.log("colonyAddress", colony.address);
      await colony.enableGSN(relayHub.address);
      // Fund hub for colony
      console.log("deposit");
      await relayHub.depositFor(colony.address, { value: WAD });
      console.log("stake");
      await relayHub.stake(RELAYER, 1200000, { value: WAD });
      console.log("register");
      await relayHub.registerRelay(1, "http://whatever.com", { from: RELAYER });
      console.log("get nonce");
      const nonce = await relayHub.getNonce(ADMIN);
      console.log(nonce);
      console.log(expenditureId, SLOT0, RECIPIENT);
      // Set expenditure recipient through GSN
      const txData = await colony.contract.methods.setExpenditureRecipient(expenditureId.toString(), SLOT0, RECIPIENT).encodeABI();
      const txHashToSign = web3.utils.soliditySha3("rlx:", ADMIN, colony.address, txData, 20, 20000000, 3000000, nonce, relayHub.address, RELAYER);
      console.log(txHashToSign);
      const prefixedMessageHash = web3.utils.soliditySha3("\x19Ethereum Signed Message:\n32", txHashToSign);
      console.log(prefixedMessageHash);
      // Refer to the local test accounts generated by ganache-cli
      const accountsJson = JSON.parse(fs.readFileSync("./ganache-accounts.json", "utf8"));
      const privKey = accountsJson.private_keys[ADMIN.toLowerCase()];

      const sigParts = ecsign(Buffer.from(prefixedMessageHash.slice(2), "hex"), Buffer.from(privKey, "hex"));
      const sig = web3Utils.bytesToHex(sigParts.r) + web3Utils.bytesToHex(sigParts.s).slice(2) + web3Utils.toHex(sigParts.v).slice(2);

      console.log(sig);
      const adminBalanceBefore = await web3.eth.getBalance(ADMIN);
      const relayerBalanceBefore = await web3.eth.getBalance(RELAYER);
      const relayerHubBalanceBefore = await relayHub.balanceOf(RELAYER);
      const colonyBalanceBefore = await relayHub.balanceOf(colony.address);

      await relayHub.relayCall(ADMIN, colony.address, txData, 20, 20000000, 3000000, nonce, sig, "0x00", {
        from: RELAYER,
        gasLimit: 9000000,
        gasPrice: 20000000
      });
      // // await colony.setExpenditureRecipient.get(expenditureId, SLOT0, RECIPIENT, { from: ADMIN });

      const expenditureSlot = await colony.getExpenditureSlot(expenditureId, SLOT0);
      console.log(expenditureSlot);
      const adminBalanceAfter = await web3.eth.getBalance(ADMIN);
      const relayerBalanceAfter = await web3.eth.getBalance(RELAYER);
      const relayerHubBalanceAfter = await relayHub.balanceOf(RELAYER);
      const colonyBalanceAfter = await relayHub.balanceOf(colony.address);

      console.log("admin:", adminBalanceBefore, adminBalanceAfter);
      console.log("relayer:", relayerBalanceBefore, relayerBalanceAfter);
      console.log("relayer on hub:", relayerHubBalanceBefore, relayerHubBalanceAfter);

      console.log("colony:", colonyBalanceBefore, colonyBalanceAfter);
    });
    ...
    it("should allow anyone to claim on behalf of the slot, in multiple tokens", async () => {
      ...
      const expenditure = await colony.getExpenditure(expenditureId);
      await colony.moveFundsBetweenPots(1, 0, 0, domain1.fundingPotId, expenditure.fundingPotId, WAD, token.address);
      await colony.moveFundsBetweenPots(1, 0, 0, domain1.fundingPotId, expenditure.fundingPotId, WAD, otherToken.address);
      await colony.finalizeExpenditure(expenditureId, { from: ADMIN });
