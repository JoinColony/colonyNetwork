...
contract("Colony Network", (accounts) => {
  ...
  describe("when creating new colonies at a specific version", () => {
    ...
    it("should allow users to create a new colony at a specific older version", async () => {
      ...
      await token.unlock();
      await colonyNetwork.createColony(token.address, oldVersion, "", "", false);
    ...
    it("should not allow users to create a new colony at a nonexistent version", async () => {
      ...
      await checkErrorRevert(colonyNetwork.createColony(token.address, nonexistentVersion, "", "", false), "colony-network-invalid-version");
    });
  });
  ...
  describe("when creating new colonies", () => {
    ...
    it("should maintain correct count of colonies", async () => {
      const token = await Token.new(...getTokenArgs());
      await token.unlock();
      await colonyNetwork.createColony(token.address, 0, "", "", false);
      await colonyNetwork.createColony(token.address, 0, "", "", false);
      await colonyNetwork.createColony(token.address, 0, "", "", false);
      await colonyNetwork.createColony(token.address, 0, "", "", false);
      await colonyNetwork.createColony(token.address, 0, "", "", false);
    ...
    it("should not allow users to create a colony with empty token", async () => {
      await checkErrorRevert(colonyNetwork.createColony(ethers.constants.AddressZero, 0, "", "", false), "colony-token-invalid-address");
    });
    ...
    it("should log a ColonyAdded event", async () => {
      const token = await Token.new(...TOKEN_ARGS);
      await expectEvent(colonyNetwork.createColony(token.address, 0, "", "", false), "ColonyAdded");
    });
  });
  ...
  describe("when managing ENS names", () => {
    ...
    it.skip("should be able to create a colony configured with the extension manager in one tx", async () => {
      const token = await Token.new(...TOKEN_ARGS);
      const { logs } = await colonyNetwork.createColony(token.address, 0, "", "", true);
      const { colonyAddress } = logs[0].args;
      ...
      await colonyNetwork.createColony(token.address, 0, "", "", true);
      await colonyNetwork.createColony(token.address, 0, "", "", true);
    ...
    it.skip("should be able to create a colony with label and configured with the extension manager in one tx", async () => {
      const token = await Token.new(...TOKEN_ARGS);
      const { logs } = await colonyNetwork.createColony(token.address, 0, "test", orbitDBAddress, true);
      const { colonyAddress } = logs[0].args;
