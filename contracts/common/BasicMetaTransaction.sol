...
abstract contract BasicMetaTransaction is DSMath, MetaTransactionMsgSender, MultiChain {
   ...
   * @param _functionSignature Signature of the actual function to be called via meta transaction
   * @param _sigR R part of the signature
   * @param _sigS S part of the signature
   * @param _sigV V part of the signature
   */
  // slither-disable-next-line locked-ether
  function executeMetaTransaction(address _user, bytes memory _functionSignature,
      bytes32 _sigR, bytes32 _sigS, uint8 _sigV) public payable returns (bytes memory) {

      require(verify(_user, getMetatransactionNonce(_user), getChainId(), _functionSignature, _sigR, _sigS, _sigV), "metatransaction-signer-signature-mismatch");
      incrementMetatransactionNonce(_user);

      // Append _user at the end to extract it from calling context
      (bool success, bytes memory returnData) = address(this).call(abi.encodePacked(_functionSignature, METATRANSACTION_FLAG, _user));
      require(success, "Function call not successful");
      emit MetaTransactionExecuted(_user, msg.sender, _functionSignature);
      return returnData;
  }
  ...
  function verify(address _owner, uint256 _nonce, uint256 _chainID, bytes memory _functionSignature,
      bytes32 _sigR, bytes32 _sigS, uint8 _sigV) public view returns (bool) {

      bytes32 hash = prefixed(keccak256(abi.encodePacked(_nonce, this, _chainID, _functionSignature)));
