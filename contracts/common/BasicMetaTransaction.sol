...
abstract contract BasicMetaTransaction is DSMath {

    event MetaTransactionExecuted(address userAddress, address payable relayerAddress, bytes functionSignature);
    bytes32 constant METATRANSACTION_FLAG = keccak256("METATRANSACTION");

    // mapping(address => uint256) private metatransactionNonces;
    ...
    function getMetatransactionNonce(address user) public virtual returns(uint256 nonce);
    ...
    function msgSender() internal view returns(address payable sender) {
        ...
        if(msg.sender == address(this) && index >= 64) {
            bytes memory array = msg.data;
            bytes32 flag;
            assembly {
                flag := mload(add(array, sub(index, 32)))
            }
