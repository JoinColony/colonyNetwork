...
contract ReputationMiningCycleRespond is ReputationMiningCycleStorage, PatriciaTreeProofs, DSMath {
  ...
  ) internal view
  {
    ...
    bytes memory disagreeStateReputationValue = new bytes(64);
    assembly {
      let x := mload(add(u, mul(32,13))) // 13 = U_DISAGREE_STATE_REPUTATION_VALUE. Constants not supported by inline solidity
      mstore(add(disagreeStateReputationValue, 0x20), x)
      x := mload(add(u, mul(32,14))) // 14 = U_DISAGREE_STATE_REPUTATION_UID. Constants not supported by inline solidity
      mstore(add(disagreeStateReputationValue, 0x40), x)
    }
    ...
    ) internal view
  {
    ...
    bytes memory previousNewReputationValue = new bytes(64);
    assembly {
      let x := mload(add(u, mul(32,15))) // 15 = U_DISAGREE_STATE_REPUTATION_VALUE. Constants not supported by inline solidity
      mstore(add(previousNewReputationValue, 0x20), x)
      x := mload(add(u, mul(32,16))) // 16 = U_DISAGREE_STATE_REPUTATION_UID. Constants not supported by inline solidity
      mstore(add(previousNewReputationValue, 0x40), x)
    }
