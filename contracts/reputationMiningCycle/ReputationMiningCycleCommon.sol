...
contract ReputationMiningCycleCommon is
  ...
  DSMath
{
  ...
  function getMinerAddressIfStaked() internal view returns (address) {
    ...
    address delegator = IColonyNetwork(colonyNetworkAddress).getMiningDelegator(
      msg.sender
    );
    require(
      delegator != address(0x00),
      "colony-reputation-mining-no-stake-or-delegator"
    );
    return delegator;
  }

  function expectedBranchMask(
    uint256 _nLeaves,
    uint256 _leaf
  ...
  function submissionWindowClosed() internal view returns (bool) {
    return
      block.timestamp - reputationMiningWindowOpenTimestamp >=
      MINING_WINDOW_SIZE;
  }
  ...
  function disputeRewardSize() internal returns (uint256) {
    ...
    uint256 rewardDenominator = nByes +
      (nUniqueSubmittedHashes - 1) *
      (2 * (3 + log2Ceiling(jrhNLeaves)) + 1);
  ...
  uint256 constant Y =
    UINT256_MAX /
      (CHALLENGE_RESPONSE_WINDOW_DURATION - ALL_ENTRIES_ALLOWED_END_OF_WINDOW);

  function responsePossible(
    DisputeStages _stage,
    uint256 _responseWindowOpened
    ...
    if (
      ...
      if (
        reputationHashSubmissions[minerAddress].proposedNewRootHash ==
        bytes32(0x00)
      ) {
        return false;
      }
      ...
      if (
        uint256(
          keccak256(abi.encodePacked(minerAddress, address(this), _stage))
        ) > target
      ) {
        return false;
      }
  ...
  function nextPowerOfTwoInclusive(uint256 _v) internal pure returns (uint) {
    // solium-disable-line security/no-assign-params
