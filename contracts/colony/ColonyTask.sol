...
contract ColonyTask is ColonyStorage {
  ...
  modifier userCanRateRole(uint256 _id, TaskRole _role) {
    ...
    if (_role == TaskRole.Manager) {
      require(
        tasks[_id].roles[uint8(TaskRole.Worker)].user == msgSender(),
        "colony-user-cannot-rate-task-manager"
      );
    } else if (_role == TaskRole.Worker) {
      require(
        tasks[_id].roles[uint8(TaskRole.Evaluator)].user == msgSender(),
  ...
  modifier ratingSecretDoesNotExist(uint256 _id, TaskRole _role) {
    ...
    require(
      taskWorkRatings[_id].secret[uint8(_role)] == "",
  ...
  modifier taskWorkRatingCommitOpen(uint256 _id) {
    ...
    require(
      block.timestamp - taskCompletionTime <= RATING_COMMIT_TIMEOUT,
  ...
  modifier taskWorkRatingRevealOpen(uint256 _id) {
    ...
    if (ratingSecrets.count == 2) {
      require(
        block.timestamp - ratingSecrets.timestamp <= RATING_REVEAL_TIMEOUT,
  ...
  modifier taskWorkRatingsComplete(uint256 _id) {
    require(
      taskWorkRatingsAssigned(_id) || taskWorkRatingsClosed(_id),
      "colony-task-ratings-incomplete"
    );
    _;
  }
  ...
  function executeTaskChange(
    ...
    require(
      _sigR.length == _sigS.length && _sigR.length == _sigV.length,
      "colony-task-change-signatures-count-do-not-match"
    );
    ...
    if (
      tasks[taskId].roles[taskRole1].user == address(0) ||
      tasks[taskId].roles[taskRole2].user == address(0)
    ) {
    ...
    require(
      _sigR.length == nSignaturesRequired,
  ...
  function executeTaskRoleAssignment(
    ...
    if (nSignaturesRequired == 1) {
      ...
      require(
        reviewerAddresses[0] == manager || reviewerAddresses[1] == manager,
        "colony-task-role-assignment-not-signed-by-manager"
      );
  ...
  function revealTaskWorkRating(
    ...
    uint8 _rating,
    bytes32 _salt
  ) public stoppable taskWorkRatingRevealOpen(_id) {
    ...
    require(
      ratingSecret == taskWorkRatings[_id].secret[uint8(_role)],
  ...
  function setAllTaskPayouts(
    ...
    require(
      evaluator == manager || evaluator == address(0x0),
  ...
  function submitTaskDeliverableAndRating(
    uint256 _id,
    bytes32 _deliverableHash,
    bytes32 _ratingSecret
  ) public stoppable {
  ...
  function deconstructRoleChangeCall(
    bytes memory _data
  ) internal pure returns (bytes4 sig, uint256 taskId, address userAddress) {
    assembly {
      sig := mload(add(_data, 0x20))
      taskId := mload(add(_data, 0x24)) // same as calldataload(72)
  ...
  function taskWorkRatingsClosed(uint256 _id) internal view returns (bool) {
    ...
    return (tasks[_id].completionTimestamp > 0 && // If this is zero, the task isn't complete yet!
      block.timestamp - tasks[_id].completionTimestamp >
      RATING_COMMIT_TIMEOUT + RATING_REVEAL_TIMEOUT);
  }
  ...
  function setTaskRoleUser(
    uint256 _id,
    TaskRole _role,
    address payable _user
  ) private taskExists(_id) taskNotComplete(_id) {
