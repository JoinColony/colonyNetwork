...
contract Colony is BasicMetaTransaction, ColonyStorage, PatriciaTreeProofs {
  ...
  function finishUpgrade() public always {
    ...
    sig = bytes4(keccak256("setExpenditurePayouts(uint256,uint256,uint256,uint256[],address,uint256[])"));
    colonyAuthority.setRoleCapability(uint8(ColonyRole.Arbitration), address(this), sig, true);

    sig = bytes4(keccak256("setExpenditureSlotPayouts(uint256,uint256,uint256,uint256,address[],uint256[])"));
    colonyAuthority.setRoleCapability(uint8(ColonyRole.Arbitration), address(this), sig, true);
