...
contract Colony is ColonyStorage, PatriciaTreeProofs {
  ...
  isInBootstrapPhase
  {
    ...
    for (uint i = 0; i < _users.length; i++) {
      ...
      require(ERC20Extended(token).transfer(_users[i], uint256(_amounts[i])), "colony-bootstrap-transfer-failed");
  ...
  function mintTokensForColonyNetwork(uint _wad) public stoppable {
    ...
    require(ERC20Extended(token).transfer(colonyNetworkAddress, _wad), "colony-token-transfer-to-network-failed");
  }
