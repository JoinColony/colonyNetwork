...
export async function getSigsAndTransactionData({ colony, taskId, functionName, signers, privKeys, sigTypes, args }) {
  ...
  const sigsPromises = sigTypes.map((type, i) => {
    if (type === 0) {
      return createSignatures(colony, ethersBNTaskId, [signers[i]], [privKeys[i]], 0, txData);
    }
    return createSignaturesTrezor(colony, ethersBNTaskId, [signers[i]], [privKeys[i]], 0, txData);
  });
