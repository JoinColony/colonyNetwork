...
if (network) {
  ...
  provider = new ethers.providers.StaticJsonRpcProvider(`http://${localProviderAddress || "localhost"}:${localProviderPort || "8545"}`);
} else {
  provider = new ethers.providers.StaticJsonRpcProvider(providerAddress[0]);
}
